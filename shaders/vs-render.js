...
void main() {
    ...
    float t1 = sin( time * timeSpeed ) * timePower;
    float t2 = sin( time * .595 * timeSpeed ) * timePower;
    float t3 = sin( time * .123 * timeSpeed ) * timePower;

    vec3 nSeed1 = vec3( sin( vPosition * noiseSize ) * vAudio.x * t1 ); 
    vec3 nSeed2 = vec3( sin( vPosition * noiseSize * .59 ) * vAudio.y * t2); 
    vec3 nSeed3 = vec3( sin( vPosition * noiseSize * 4.) * vAudio.z * t3); 

    vec3 seed = nSeed1 + nSeed2 + nSeed3;
    vNoise = abs( snoise( seed ) );
